...
@Component
public class CobroAutomaticoTrimestralReader extends JdbcCursorItemReader<OutstandingEntity> implements ItemReader<OutstandingEntity> {


    public CobroAutomaticoTrimestralReader(@Autowired DataSource primaryDataSource) {
        setDataSource(primaryDataSource);
        setSql("SELECT * FROM outstanding where tipo_cobros like 'AUTOMATICO' AND cantidad_restante != 0 AND periodicidad like 'TRIMESTRAL'");
        setFetchSize(100);
        setRowMapper(new CobroAutomaticoTrimestralReader.OutstandingRowMapper());
    }
