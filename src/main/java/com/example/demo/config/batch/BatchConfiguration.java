...
public class BatchConfiguration {
	...
	@Bean
	public Job cobroAutomaticoSemanalJob(JobRepository jobRepository,
			@Qualifier("stepCobroAutomaticoSemanal") Step step) {
		return new JobBuilder("cobroAutomaticoSemanalJob", jobRepository).flow(step).end().build();
	}
	...
	public Step stepCobroAutomaticoSemanal(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
		return new StepBuilder("stepCobroAutomaticoSemanal", jobRepository)
				...
				.reader(cobroAutomaticoSemanalReader).processor(cobroAutomaticoProcessor).writer(cobroAutomaticoWriter)
				.build();
	}

	@Bean
	public Job cobroAutomaticoMensualJob(JobRepository jobRepository,
			@Qualifier("stepCobroAutomaticoMensual") Step step) {
		return new JobBuilder("cobroAutomaticoMensualJob", jobRepository).flow(step).end().build();
	}
	...
	public Step stepCobroAutomaticoMensual(JobRepository jobRepository, PlatformTransactionManager transactionManager) {
		return new StepBuilder("stepCobroAutomaticoMensual", jobRepository)
				.<OutstandingEntity, OutstandingEntity>chunk(50, transactionManager)
				.reader(cobroAutomaticoMensualReader).processor(cobroAutomaticoProcessor).writer(cobroAutomaticoWriter)
				.build();
	}

	@Bean
	public Job cobroAutomaticoTrimestralJob(JobRepository jobRepository,
			@Qualifier("stepCobroAutomaticoTrimestral") Step step) {
		return new JobBuilder("cobroAutomaticoTrimestralJob", jobRepository).flow(step).end().build();
	}

	@Bean
	@Qualifier("stepCobroAutomaticoTrimestral")
	public Step stepCobroAutomaticoTrimestral(JobRepository jobRepository,
			PlatformTransactionManager transactionManager) {
		return new StepBuilder("stepCobroAutomaticoTrimestral", jobRepository)
				.<OutstandingEntity, OutstandingEntity>chunk(50, transactionManager)
				.reader(cobroAutomaticoTrimestralReader).processor(cobroAutomaticoProcessor)
				.writer(cobroAutomaticoWriter).build();
	}
