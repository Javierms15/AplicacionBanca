...
@Component
public class CobroAutomaticoMensualReader extends JdbcCursorItemReader<OutstandingEntity> implements ItemReader<OutstandingEntity> {


    public CobroAutomaticoMensualReader(@Autowired DataSource primaryDataSource) {
        setDataSource(primaryDataSource);
        setSql("SELECT * FROM outstanding where tipo_cobros like 'AUTOMATICO' AND cantidad_restante != 0 AND periodicidad like 'MENSUAL'");
        setFetchSize(100);
        setRowMapper(new CobroAutomaticoMensualReader.OutstandingRowMapper());
    }
